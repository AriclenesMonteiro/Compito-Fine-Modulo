...
function eseguiRicerca() {
    console.log("Primo pass: Clicato il bottone ");

   
    let titoloCercato = document.getElementById("inputTitolo").value.toLowerCase();
    let luogoCercato = document.getElementById("inputLuogo").value.toLowerCase();
    console.log("Secondo pass: Valori raccolti ->", titoloCercato, luogoCercato);

    let risultatiFiltrati = [];

    
    for (let i = 0; i < jobs.length; i++) {
        let lavoroSingolo = jobs[i];
        
       
        if (lavoroSingolo.title.toLowerCase().includes(titoloCercato) && 
            lavoroSingolo.location.toLowerCase().includes(luogoCercato)) {
            risultatiFiltrati.push(lavoroSingolo);
        }
    }
    console.log("Terzo pass: Lavori trovati ->", risultatiFiltrati.length); 

    
    document.getElementById("risultatoContatore").innerText = "Risultati trovati: " + risultatiFiltrati.length;

    
    let listaUl = document.getElementById("listaLavori");
    listaUl.innerHTML = "";

    for (let j = 0; j < risultatiFiltrati.length; j++) {
        let li = document.createElement("li");
        li.innerText = risultatiFiltrati[j].title + " - " + risultatiFiltrati[j].location;
        listaUl.appendChild(li);
    }
    console.log("Quarto pass: aggiornata!");
}